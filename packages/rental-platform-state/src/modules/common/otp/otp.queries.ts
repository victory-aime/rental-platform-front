...
const generateOtpMutation = (args: TYPES.QUERIES.MutationPayload<TYPES.MODELS.COMMON.OTP.IOtp>) => {
...
const renewOtpMutation = (args: TYPES.QUERIES.MutationPayload<TYPES.MODELS.COMMON.OTP.IOtp>) => {
  return TYPES.QUERIES.useCustomMutation<TYPES.MODELS.COMMON.OTP.IOtp, any, AxiosError>({
    mutationKey: [Constants.OTP_KEYS.RENEW_OTP],
    mutationFn: ({ payload }) => otpServiceInstance().renewOtp(payload!),
    options: args.mutationOptions,
  })
}
...
export { generateOtpMutation, renewOtpMutation, validateOtpMutation }
